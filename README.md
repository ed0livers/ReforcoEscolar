...
server/index.ts (O Chefe de Cozinha): É o arquivo que liga o servidor. Ele abre a porta (3001) para o Frontend fazer pedidos e delega os pedidos para as rotas corretas (ex: "Se pedirem /api/alunos, mande para a rota de alunos").
server/db.ts (O Estoquista): É o arquivo que sabe como se conectar ao MySQL usando a senha e o usuário que estão no arquivo .env.
server/auth.ts (O Segurança): Cuida do registro de usuários (quem cria a conta), do login e verifica as senhas. Ele cria o Token JWT (uma "pulseirinha VIP") que prova que o usuário está logado.
server/routes/... (Os Cozinheiros Especializados): Cada arquivo (como alunos.ts, turmas.ts) sabe como ler, criar, editar e deletar (CRUD) uma informação específica no banco de dados.
No Frontend (O Salão - src/)
src/api/ConectorAPI.ts (O Garçom Interativo): Esta é uma das partes mais importantes. O Frontend não fala diretamente com o Banco de Dados. O ConectorAPI sabe exatamente como empacotar os dados digitados pelo usuário, colar a "pulseirinha VIP" (Token JWT) e enviar para o Backend através de requisições fetch.
src/api/SincronizadorDeDados.tsx (O Administrador do Salão): Ele usa um Hook chamado useSincronizador. O papel dele é:
...
src/hooks/useAuth.ts (O Porteiro da Tela): Verifica se o usuário tem a "pulseirinha VIP" no localStorage (memória do navegador). Se não tiver, expulsa o usuário de volta para a tela de Login.
src/App.tsx (A Planta do Restaurante): Define que tela deve aparecer em qual endereço (Rotas). Se digitar /alunos, ele desenha a tela de Alunos na página.
...
A Ação: O arquivo src/pages/Alunos.tsx pega o nome e a turma que você digitou e diz: "Sincronizador, adiciona esse aluno para mim!" (Chamando a função addAluno).
O Pedido: O SincronizadorDeDados.tsx pega esses dados e chama o garçom: "ConectorAPI, insere isso lá na rota de alunos!" (Fazendo ConectorAPI.from('alunos').insert(...)).
A Viagem: O ConectorAPI.ts cria uma chamada HTTP (POST para /api/alunos), grampeia seu Token JWT no cabeçalho e envia pela internet (mesmo que localmente).
A Recepção Segura: O server/index.ts recebe o pedido, o segurança (auth em alunos.ts) olha o JWT e diz: "Ele tem a pulseira, pode passar."
A Gravação: O server/routes/alunos.ts converte esse pedido numa linguagem SQL (INSERT INTO alunos...) e o db.ts joga isso no MySQL.
A Resposta: O MySQL diz "Salvo com sucesso". O Backend devolve o novo aluno para o Garçom (ConectorAPI).
A Atualização: O SincronizadorDeDados recebe a confirmação e atualiza toda a tela com o novo aluno mágico e instantaneamente.
