    ...
    <motion.section key="configs" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-4xl mx-auto space-y-6 py-6">
      <div className="flex items-center justify-between mb-2">
        <div>
          ...
          <p className="text-xs text-purple-400 font-bold">Gerencie sua unidade e segurança de acesso.</p>
        </div>
        <Settings className="text-purple-200 w-8 h-8" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* UNIDADE SETTINGS */}
        <div className="bg-white p-8 rounded-3xl border border-purple-50 shadow-sm space-y-6">
          <h4 className="font-black text-sm text-purple-900 flex items-center gap-2">
            <Layers className="w-4 h-4" /> Dados da Unidade
          </h4>
          <div className="space-y-4">
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-purple-300">Nome da Escola</label>
              <input id="cfg_name" type="text" defaultValue={configs?.unit_name} className="w-full p-3 bg-purple-50/50 border border-purple-100 rounded-xl font-bold text-sm text-purple-900 outline-none focus:ring-2 focus:ring-purple-200" />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-purple-300">WhatsApp</label>
              <input placeholder="(00) 0 0000-0000" defaultValue={configs?.whatsapp} id="cfg_whatsapp" className="w-full p-3 bg-purple-50/50 border border-purple-100 rounded-xl text-sm font-bold text-gray-500 outline-none" />
            </div>
            <button 
               onClick={async () => {
               ...
               className="w-full py-3 bg-[#8126cf] text-white rounded-xl font-black text-sm shadow-lg shadow-purple-50 hover:bg-[#6b1ead] transition-all"
             >
               Salvar Unidade
            </button>
          </div>
        ...
        <div className="bg-zinc-50 p-8 rounded-3xl border border-zinc-100 shadow-sm space-y-6">
          ...
          <form onSubmit={async (e) => {
            ...
            const { error: signInError } = await ConectorSupabase.auth.signInWithPassword({ email: userEmail, password: oldPass });
            if (signInError) { alert("Senha atual inválida!"); return; }

            const { error } = await ConectorSupabase.auth.updateUser({ email: nEmail });
            if (error) { alert(`Erro: ${error.message}`); }
            else { addToast("✅ Confirmado! Verifique seu novo e-mail."); form.reset(); }
          ...
          <form onSubmit={async (e) => {
            ...
            const { error: signInError } = await ConectorSupabase.auth.signInWithPassword({ email: userEmail, password: oldPass });
            if (signInError) { alert("Dados atuais inválidos!"); return; }

            const { error } = await ConectorSupabase.auth.updateUser({ password: nPass });
            if (error) { alert(`Erro: ${error.message}`); }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <input name="nPass" type="password" placeholder="Nova Senha" className="w-full p-2.5 bg-purple-50 border border-purple-100 rounded-xl text-[11px] font-bold outline-none" required />
              <input name="nPassConfirm" type="password" placeholder="Confirmar Nova" className="w-full p-2.5 bg-purple-50 border border-purple-100 rounded-xl text-[11px] font-bold outline-none" required />
            </div>
